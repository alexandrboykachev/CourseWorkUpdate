...
public:
	Flow(ifstream& file)
	{
		...
		while (!file.eof()) {
			...
			if (s1.size() >= 5) { 
				if (!((s1[0] >= 'A' && s1[0] <= 'Z') && (s1[1] == ' '))) {
					throw exception(string(("Error entering the first character in the string or missing a space after it. Check the correctness of the input in the file and correct these errors in the line under the number: " + to_string(str_num))).c_str());
				}
				if (!((s1[2] >= 'A' && s1[2] <= 'Z') && (s1[3] == ' '))) {
					throw exception(string(("Error entering the second character in the string or missing a space after it. Check the correctness of the input in the file and correct these errors in the line under the number: " + to_string(str_num))).c_str());
				}
				string cur;
				for (size_t i = 4; i < s1.size(); ++i) {
					if (s1[i] >= '0' && s1[i] <= '9')
						cur += s1[i];
					else {
						throw exception(string(("Error entering the third character (bandwidth) in the string or the presence of a space after it.Please note that the bandwidth cannot be negative. Check that you entered the file correctly and correct these errors in the line number: " + to_string(str_num))).c_str());
					}
				}
			...
			{
				if (str_num == 1)
					throw exception("A data-entry error. File is empty.");
				else
				throw exception(string(("A data-entry error. Check the correctness of the input in the file and correct these errors in the line under the number: " + to_string(str_num))).c_str());
			}
			++str_num;
		}
		...
		while (!file.eof()) {
			...
			if (vert1 == vert2)
				throw exception(string("The path from the vertex to itself is impossible in the string under the number: " + to_string(str_num)).c_str());
