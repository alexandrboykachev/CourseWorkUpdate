...
public:
	...
	{
	public:
		Node(bool color = COLOR_RED, T key = T(), Node* parent = nullptr, Node* left = nullptr, Node* right = nullptr, T1 value = T1()) :color(color), key(key), parent(parent), left(left), right(right), value(value) {}
		T key;
		T1 value;
		bool color;
		Node* parent;
	...
	Map(Node* Root = nullptr, Node* TNULL = new Node(0)) :Root(TNULL), TNULL(TNULL) {}
	...
	void printTree()
	{
		...
		else throw out_of_range("Tree is empty!");
	}
